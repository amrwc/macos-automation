...
def should_have_executed(monkeypatch, argv: List[str]) -> None:
    ...
    result: str = Tunnelblick().execute()
    assert result == 'true'
...
def should_have_parsed_argv(monkeypatch, argv: List[str]) -> None:
    tunnelblick: Tunnelblick = Tunnelblick(argv)
    assert tunnelblick.argv == argv
...
def should_have_gotten_config(monkeypatch) -> None:
    ...
    with patch('builtins.open', mock_open(read_data='data')):
        result: Dict[str, str] = Tunnelblick().get_config(config_path)
...
def should_have_built_apple_script(monkeypatch) -> None:
    ...
    result: str = Tunnelblick().build_apple_script(instruction)
